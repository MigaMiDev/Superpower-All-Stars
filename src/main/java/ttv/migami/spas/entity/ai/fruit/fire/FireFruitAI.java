...
public class FireFruitAI {

    @SubscribeEvent
    public void onLivingUpdate(LivingEvent.LivingTickEvent event) {
        if (event.getEntity() instanceof PathfinderMob skeleton) {

            if (skeleton.getTags().contains("FireyBoi") && !(skeleton.hasEffect(ModEffects.FIRE_FRUIT.get()))) {
                skeleton.addEffect(new MobEffectInstance(ModEffects.FIRE_FRUIT.get(), -1, 0, false, true));
            }

            if (skeleton.hasEffect(ModEffects.FIRE_FRUIT.get())) {

                boolean hasTrumpetGoal = skeleton.goalSelector.getAvailableGoals().stream()
                        .anyMatch(prioritizedGoal -> prioritizedGoal.getGoal() instanceof FireFruitAttackGoal);
                ...
                if (!hasTrumpetGoal) {
                    ...
                    skeleton.goalSelector.addGoal(4, new FireFruitAttackGoal(skeleton, 1.0D, 60, 12F));
