package ttv.migami.nep.entity.fx;
...
public class InkMarkEntity extends Entity {
    private static final EntityDataAccessor<Float> SCALE = SynchedEntityData.defineId(InkMarkEntity.class, EntityDataSerializers.FLOAT);

    public int life = 200;
    private int tickCounter = 0;
    private float randomRotation = 0;

    public InkMarkEntity(EntityType<?> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public InkMarkEntity(Level pLevel, BlockPos blockPos, int life, float size) {
        super(ModEntities.INK_MARK.get(), pLevel);
    ...
    public static void summonInkMark(Level level, BlockPos pos, int life, float size) {
        if (!level.isClientSide) {
            InkMarkEntity scorchMark = new InkMarkEntity(level, pos, life, size);
