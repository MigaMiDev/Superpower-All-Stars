package ttv.migami.nep.entity.ai.fruit.fire;
...
import ttv.migami.nep.entity.ai.FruitAttackGoal;
...
import static ttv.migami.nep.common.network.ServerPlayHandler.actionSlowdown;
import static ttv.migami.nep.common.network.ServerPlayHandler.rayTrace;

public class FireFruitAttackGoal extends FruitAttackGoal<PathfinderMob> {
    ...
    public FireFruitAttackGoal(PathfinderMob mob, double speedModifier, int attackIntervalMin, float attackRadius) {
    ...
    private void shootBone(LivingEntity target, int amount) {
        ...
        if (entityHitResult != null) {
            //bone = new Fireball(this.mob.level(), this.mob, skeletonPos.add(0, 1, 0), target.getEyePosition().add(0, 1, 0));
        }
        else {
            //bone = new Fireball(this.mob.level(), this.mob, skeletonPos, blockPos.getCenter());
        }
