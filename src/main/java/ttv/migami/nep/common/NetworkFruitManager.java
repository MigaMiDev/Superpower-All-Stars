...
import ttv.migami.nep.annotation.Validator;
                ...
                resources.forEach(resourceLocation ->
                {
                    ...
                    manager.getResource(resourceLocation).ifPresent(resource ->
                    {
                        try(Reader reader = new BufferedReader(new InputStreamReader(resource.open(), StandardCharsets.UTF_8)))
                        {
                            ...
                            {
                                NotEnoughPowers.LOGGER.error("Couldn't load data file {} as it is missing or malformed. Using default fruit data", resourceLocation);
                                map.putIfAbsent((FruitEffect) item, new Fruit());
                            }
                        }
                        catch(InvalidObjectException e)
                        {
                            NotEnoughPowers.LOGGER.error("Missing required properties for {}", resourceLocation);
                            e.printStackTrace();
                        }
                        catch(IOException e)
                        {
                            NotEnoughPowers.LOGGER.error("Couldn't parse data file {}", resourceLocation);
                        }
